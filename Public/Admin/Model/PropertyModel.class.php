...
/************************************************ 审核管理模块 ****************************************/
	...
	public function get_property_list(){
        ...
        $data = $model
        	->field("b.*,p.property_id,c.customer_company,c.customer_name,p.property_total")
        ...
        foreach($data as $k=>$v){
        	...
        	if(empty($v['business_wfk'])){
                $data[$k]['wfk'] = $v['business_total'];
            }
        }
        $arr["total"] = $count;
        $arr["property"] = $data;
	...
	public function get_property_save(){
		...
		$res2["business_fkx"] = '';
        if(empty($business['business_wfk'])){
		    $res2["business_wfk"] = $business['business_total']-$pro['property_total'];
        }else{
            $res2["business_wfk"] = $business["business_wfk"]-$pro['property_total'];
        }
