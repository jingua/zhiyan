...
<div class="page-container" style="margin-top:-30px;">
	<div class="cl pd-5 mt-20">
		<form action="<?php echo U('Member/business_member');?>" method="post">
		<!-- date -->
		<input type="text" 
		...
		style="width:200px;" value="<?php echo ($from); ?>"> 
		-
		<input type="text" 
		onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',Date:'#F<?php echo ($dp["$D('datemin') or '%y-%M-%d'"]); ?>'})" 
		id="datemin" class="input-text Wdate" placeholder="请选择添加时间" value="<?php echo ($to); ?>" 
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody id="ks_user">
			<?php if(is_array($business)): foreach($business as $key=>$v): ?><tr class="text-c">
			...
			<?php if(is_array($member)): foreach($member as $key=>$v2): ?><tr class="text-c">
				<td><?php echo ($v2["member_id"]); ?>+<?php echo ($v2["parent_id"]); ?></td>
				<td><?php echo ($v2["member_name"]); ?>+<?php echo ($v2["parent_name"]); ?></td>
				<td>
...
$(".excel_down_xls").click(function(){
    ...
    window.location.href="/zhiyan/business_admin/index.php?s=/Admin/Member/excel.html&from=" + from + '&to=' + to;    
});
