...
<div class="page-container" style="margin-top:-30px;">
	<div class="cl pd-5 mt-20">
		...
		style="width:200px;" value="<?php echo ($log["from"]); ?>"> 
		-
		<input type="text" 
		onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',Date:'#F<?php echo ($dp["$D('datemin') or '%y-%M-%d'"]); ?>'})" 
		id="datemin" class="input-text Wdate" placeholder="请选择添加时间" value="<?php echo ($log["to"]); ?>" 
		style="width:200px;" name="to">
		<!-- button -->
		<button type="submit" class="btn btn-success radius" id="sousuo_user">
		<i class="Hui-iconfont">&#xe665;</i> 搜索日志</button>
		</form>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20" style="margin-top:6px;"> 
		...
		<span class="r" style="line-height:30px;">
			共有数据&nbsp;&nbsp;
			<strong><?php echo ($log["total"]); ?></strong>&nbsp;&nbsp;条
		</span> 
	</div>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody id="ks_user">
			<?php if(is_array($log["log"])): foreach($log["log"] as $key=>$v): ?><tr class="text-c">
				<td>
					<input type="checkbox" value="<?php echo ($v["log_id"]); ?>" name="dataid" 
					id="dataid" _id="<?php echo ($v["log_id"]); ?>">
				</td>
				<td><?php echo ($v["log_id"]); ?></td>
				<td><?php echo ($v["log_text"]); ?></td>
				<td><?php echo ($v["admin_name"]); ?></td>
				<td><?php echo ($v["log_time"]); ?></td>
			</tr><?php endforeach; endif; ?>
			<tr><td colspan=16 style="text-align:center;"><?php echo ($log["link"]); ?></td></tr>
		</tbody>
	</table>
	</div>
</div>
...
<script>
var page_url = {
    'save_del' : "<?php echo U('Log/log_del');?>",
    'list_url' : "<?php echo U('Log/log_list');?>",
}
</script>