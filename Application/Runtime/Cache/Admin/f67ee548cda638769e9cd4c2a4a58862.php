...
<nav class="breadcrumb">
	...
	<span class="c-gray en">&gt;</span> 支付方式列表 
	<!-- 刷新 -->
...
<div class="page-container" style="margin-top:-30px;">
	<div class="cl pd-5 mt-20">
		<form action="<?php echo U('Payment/pay_list');?>" method="post">
		<!-- text -->
		<input type="text" class="input-text" value="<?php echo ($pay["pay_name"]); ?>"
		style="width:280px" name="pay_name" placeholder="请输入支付方式名称" />
		<!-- date -->
		<input type="text" 
		...
		style="width:200px;" value="<?php echo ($pay["from"]); ?>"> 
		-
		<input type="text" 
		onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',Date:'#F<?php echo ($dp["$D('datemin') or '%y-%M-%d'"]); ?>'})" 
		id="datemin" class="input-text Wdate" placeholder="请选择添加时间" value="<?php echo ($pay["to"]); ?>" 
		style="width:200px;" name="to">
		<!-- button -->
		<button type="submit" class="btn btn-success radius" id="sousuo_user">
		<i class="Hui-iconfont">&#xe665;</i> 搜索支付方式</button>
		</form>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20" style="margin-top:6px;"> 
		<span class="l">
			...
			<!-- add -->
			<a href="<?php echo U('Payment/pay_add');?>" class="btn btn-success more_coupon">添加支付方式</a>
		</span> 
		<!-- total -->
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody id="ks_user">
			<?php if(is_array($pay["pay"])): foreach($pay["pay"] as $key=>$v): ?><tr class="text-c">
				<td>
					<input type="checkbox" value="<?php echo ($v["pay_id"]); ?>" name="dataid" 
					id="dataid" _id="<?php echo ($v["pay_id"]); ?>">
				</td>
				...
				<td class="td-manage">
					<!-- edit -->
					<a title="编辑" 
						href="<?php echo U('Payment/pay_mod',array('pay_id'=>$v['pay_id']));?>" 
						class="ml-5" style="text-decoration:none">
						<i class="Hui-iconfont">&#xe6df;</i>
					</a>&nbsp;&nbsp;
					<!-- delete -->
					<a style="text-decoration:none" class="ml-5" 
						onClick="del(this,<?php echo ($v['pay_id']); ?>)" 
						href="javascript:;" title="删除">
						<i class="Hui-iconfont">&#xe6e2;</i>
					</a>
				</td>
			</tr><?php endforeach; endif; ?>
			<tr><td colspan=13 style="text-align:center;"><?php echo ($pay["link"]); ?></td></tr>
		</tbody>
	</table>
	</div>
</div>
...
<script>
var page_url = {
    'save_del' : "<?php echo U('Payment/pay_del');?>",
