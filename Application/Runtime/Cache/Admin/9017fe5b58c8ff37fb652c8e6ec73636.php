...
<div class="page-container" style="margin-top:-30px;">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20" style="margin-top:6px;"> 
		<span class="l">
			...
			<!-- add -->
			<a href="<?php echo U('Business/sign_add');?>" class="btn btn-success more_coupon">添加签约</a>
		</span> 
		<!-- total -->
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody id="ks_user">
			<?php if(is_array($sign["sign"])): foreach($sign["sign"] as $key=>$v): ?><tr class="text-c">
				<td>
					<input type="checkbox" value="<?php echo ($v["sign_id"]); ?>" name="dataid" 
					id="dataid" _id="<?php echo ($v["sign_id"]); ?>">
				</td>
				...
				<td>
					<?php if(($v["sign_status"] == 1)): ?><span class="label label-danger radius">审核中</span>
					<?php elseif($v["sign_status"] == 2): ?>
						<span class="label label-success radius">审核通过</span>
					<?php else: ?> 
						<span class="label label-danger radius">审核未通过</span><?php endif; ?>
				</td>
				<td><a href="<?php echo ($v["sign_pic"]); ?>">查看合同</a></td>
				<td>
					<!-- 编辑 -->
					<a style="text-decoration:none" class="ml-5" 
					href="<?php echo U('Business/sign_mod',array('sign_id'=>$v['sign_id']));?>" title="分配角色">
					<i class="Hui-iconfont">&#xe6df;</i>
					</a>
					<!-- delete -->
					<a style="text-decoration:none" class="ml-5" 
						onClick="del(this,<?php echo ($v['sign_id']); ?>)" 
						href="javascript:;" title="删除">
						<i class="Hui-iconfont">&#xe6e2;</i>
					</a>
				</td>
			</tr><?php endforeach; endif; ?>
			<tr><td colspan=13 style="text-align:center;"><?php echo ($sign["link"]); ?></td></tr>
		</tbody>
	</table>
	</div>
</div>
...
var page_url = {
    'save_del' : "<?php echo U('Business/sign_del');?>",
