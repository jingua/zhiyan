...
<nav class="breadcrumb">
    ...
    <span class="c-gray en">&gt;</span> 编辑节点
    <!-- 刷新 -->
...
<article class="page-container">
	<form class="form form-horizontal" id="form-article-add" method="post" enctype="multipart/form-data">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
				<select name="node_pid" class="select">
					<?php if(is_array($actions)): foreach($actions as $key=>$v): if($v['node_id']==$node['node_pid']): ?><option value="<?php echo ($v["node_id"]); ?>" selected="selected">
			            	<?php echo ($v["node_name"]); ?>Action-<?php echo ($v["node_descr"]); ?></option>
			            <?php else: ?>	
			            	<option value="<?php echo ($v["node_id"]); ?>" 
			            	<?php if(($v["node_id"]) == $node["node_id"]): ?>selected<?php endif; ?>>
			            	<?php echo ($v["node_name"]); ?>Action-<?php echo ($v["node_descr"]); ?></option><?php endif; endforeach; endif; ?>
				</select>
				</span> 
			</div>
		</div>
		...
		<div class="row cl">
			...
			<input type="text" class="input-text" value="<?php echo ($node["node_name"]); ?>" 
			placeholder="" id="node_name" name="node_name">
			</div>
		</div>
		<!-- text -->
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($node["node_descr"]); ?>" 
				placeholder="" id="node_descr" name="node_descr">
				<input type="hidden" name="node_id" value="<?php echo ($node["node_id"]); ?>">
			</div>
		</div>
		<!-- text -->
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($node["node_sort"]); ?>" 
				placeholder="" id="node_sort" name="node_sort">
			</div>
		</div>
		<!-- button -->
        <div class="row cl">
...
<script>
var page_url = {
    'save_url' : "<?php echo U('Node/node_edit');?>",
