...
<div class="page-container" style="margin-top:-30px;">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20" style="margin-top:6px;"> 
		<span class="l">
			...
			<!-- add -->
			<a href="<?php echo U('Channel/channel_add');?>" class="btn btn-success more_coupon">添加渠道</a>
		</span> 
		<!-- total -->
		...
		<tbody id="ks_user">
			<?php if(is_array($channel["channel"])): foreach($channel["channel"] as $key=>$v): ?><tr class="text-c">
				...
				<td class="td-manage">
					<!-- edit -->
					<a title="编辑" 
						href="<?php echo U('Channel/channel_mod',array('channel_id'=>$v['channel_id']));?>" 
						class="ml-5" style="text-decoration:none">
						<i class="Hui-iconfont">&#xe6df;</i>
					</a>&nbsp;&nbsp;
					<!-- delete -->
					<?php if(($v["channel_redel"]) != "2"): ?><a style="text-decoration:none" class="ml-5" 
							onClick="redel(this,<?php echo ($v['channel_id']); ?>)" href="javascript:;" title="删除申请">
							<span class="label label-success radius">删除申请</span>
						</a><?php endif; ?>
					<?php if(($v["channel_redel"]) == "2"): ?><a style="text-decoration:none" class="ml-5" 
							onClick="dels(this,<?php echo ($v['channel_id']); ?>)" href="javascript:;" title="删除申请">
...
<script>
var page_url = {
    'save_del' : "<?php echo U('Channel/channel_del');?>",
...
function redel(obj,id){
  layer.confirm("您确定要申请删除吗",function(index){
	$.ajax({
		type: "post",
		url: "<?php echo U('Channel/channel_del');?>",
		data: {"id":id},
		dataType: "json",
		success:function(data){
			if(data.ok==200){
				layer.alert(data.info,{icon:1,time:1000});
				setInterval(function(){ location.href="<?php echo U('Channel/Channel_list');?>"; },1000);
			}else{
...
function dels(obj,id){
  layer.confirm("您确定要撤消删除吗",function(index){
	$.ajax({
		type: "post",
		url: "<?php echo U('Channel/channel_redel');?>",
		data: {"id":id},
		dataType: "json",
		success:function(data){
			if(data.ok==200){
