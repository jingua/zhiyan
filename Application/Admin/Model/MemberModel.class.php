...
/************************************************ 成员管理模块 ****************************************/
	...
	public function get_member_mod_save(){
		...
		$map1['admin_id'] = $_POST['member_id'];
		M("userRole")->where($map1)->delete();
		
		$res1['admin_id'] = $_POST['member_id'];
		$res1['role_id'] = $_POST['role_id'];
		M("userRole")->add($res1);
	...
	public function get_member_parent(){
		...
		$member = M("member")->where("member_del=1 and parent_id in($id)")->select();
		foreach($member as $k33 => $v33){
			$member[$k33]['parent_name'] = M("member")->where("member_id='{$v33['parent_id']}'")->getField("member_name");
			$member[$k33]['id'] = $v33['member_id'].",".$v33['parent_id'];
			$member[$k33]['name'] = $v33['member_name']."+".M("member")->where("member_id='{$v33['parent_id']}'")->getField("member_name");
		}
		foreach($member as $k1 => $v1){
			//绑定关系第一次循环 begin 
			$member[$k1]['business'] = $model
			...
			///////////////////////////////////////////////////////注释说明
			$member[$k1]['member_text'] = "注：\n1.公众号名称务必填写完整\n2.已经有填写内容的选项框会自动根据业务人员填写的数据自动生成相应的正确结果\n3.尾款日期一定要按照所示填写\n4.如若业务量过大，表格不够填写，可直接在表格下方添加若干行，直接下拉带有公式的单元格即可复制相应的计算公式\n5.该表格为试行阶段，欢迎大家多多提建议，有特殊情况麻烦告知，谢谢\n6.将年匡客户名称填充";
			//绑定关系第一次循环 end
			//绑定关系第二次循环 begin
			$member[$k1]['business_parent'] = $model
				->field("t.*,b.remind_time,b.business_wfk,b.business_total,b.contract_id,b.member_id,b.customer_id,b.business_fkx,b.update_time,b.pay_id,b.create_time")
				->table("zhiyan_business b, zhiyan_business_time t")
				->where("b.business_del=1 and b.business_id=t.business_id and b.member_id='{$v1['parent_id']}'".$str['str'])
				->select();
				foreach($member[$k1]['business_parent']  as $k2 => $v2){
					$member[$k1]['business_parent'][$k2]['remind_time'] = date("Y年m月",$v2['remind_time']);//尾款日期
					$member[$k1]['business_parent'][$k2]['time_time'] = date("Y/m/d",$v2['time_time']);//投放时间
					$member[$k1]['business_parent'][$k2]['update_time'] = date("Y/m/d",$v2['update_time']);//预付日期
					$member[$k1]['business_parent'][$k2]['business_tfcs'] = 1; //投放次数
					$member[$k1]['business_parent'][$k2]['business_cjje'] = $v2['business_total']-$v2['invoice_price'];//成交金额
					$member[$k1]['business_parent'][$k2]['pay_name'] = M("payment")->where("pay_id='{$v2['pay_id']}'")->getField("pay_name");//支付方式
					$pay = M("pay")
						->where("pay_del=1 and pay_status=2 and business_id='{$v2['business_id']}'")
						->field("pay_price,sum(pay_price) as pay_total")
						->select();
					$member[$k1]['business_parent'][$k2]['business_fandian'] = $pay['0']['pay_total'];//返点
					if($v2['business_total'] == $v2['business_wfk']){
						$member[$k1]['business_parent'][$k2]['business_fkqk'] = 0;//预付情况
						$member[$k1]['business_parent'][$k2]['business_weikuan'] = $v2['business_total'];//尾款
					}else{
						$member[$k1]['business_parent'][$k2]['business_fkqk'] = $v2['business_total']-$v2['business_wfk'];//预付情况
						$member[$k1]['business_parent'][$k2]['business_weikuan'] = $v2['business_total']-$member[$k1]['business_parent'][$k2]['business_fkqk'];//尾款
					}
					//判断发放或待申请 
					if($v2['business_wfk'] == 0){
						$member[$k1]['business_parent'][$k2]['business_wfk'] = "发放";
					}else{
						$member[$k1]['business_parent'][$k2]['business_wfk'] = "待申请";
					}
					//查询渠道 
					$member[$k1]['business_parent'][$k2]['channel_name'] = M("channel")
						->where("channel_id='{$v2['channel_id']}'")
						->getField("channel_name");
					//查询成员所有渠道编号 
					$channel = M("channel")
					...
					if(in_array($v2['channel_id'],$channel)){
						if($v2['time_text'] == 'first'){
							$member[$k1]['business_parent'][$k2]['time_text'] = "头条";
							$member[$k1]['business_parent'][$k2]['zhibiao'] = 2;
							$member[$k1]['business_parent'][$k2]['zhibiao_dc'] = 2*$v2['time_price'];
							$member[$k1]['business_parent'][$k2]['time_total'] = M("channel")
								->where("channel_id='{$v2['channel_id']}'")
								->getField("price_first"); //总金额
						}else{
							$member[$k1]['business_parent'][$k2]['time_text'] = "次条";
							$member[$k1]['business_parent'][$k2]['zhibiao'] = 1;
							$member[$k1]['business_parent'][$k2]['zhibiao_dc'] = 1*$v2['time_price'];
							$member[$k1]['business_parent'][$k2]['time_total'] = M("channel")
								->where("channel_id='{$v2['channel_id']}'")
								->getField("price_end"); //总金额
						}
						...
						if($v2['time_text'] == 'first'){
							$member[$k1]['business_parent'][$k2]['time_text'] = "头条";
							$member[$k1]['business_parent'][$k2]['zhibiao'] = 2*0.8;
							$member[$k1]['business_parent'][$k2]['zhibiao_dc'] = 2*$v2['time_price']*0.8; //指标达成
							$member[$k1]['business_parent'][$k2]['time_total'] = M("channel")
								->where("channel_id='{$v2['channel_id']}'")
								->getField("price_first"); //总金额
						}else{
							$member[$k1]['business_parent'][$k2]['time_text'] = "次条";
							$member[$k1]['business_parent'][$k2]['zhibiao'] = 1*0.8;
							$member[$k1]['business_parent'][$k2]['zhibiao_dc'] = 1*$v2['time_price']*0.8; //指标达成
							$member[$k1]['business_parent'][$k2]['time_total'] = M("channel")
								->where("channel_id='{$v2['channel_id']}'")
								->getField("price_end"); //总金额
						}
				...
				//查询客户信息 
				$customer = M("customer")->where("customer_id='{$v2['customer_id']}'")->find();
				$member[$k1]['business_parent'][$k2]['customer_company'] = $customer['customer_company']; //对接公司
				$member[$k1]['business_parent'][$k2]['customer_brand'] = $customer['customer_brand']; //品牌
				$member[$k1]['business_parent'][$k2]['deal_money'] = $v['business_total']*$v['time_price']; //
				if($v2['contract_id']){
					$member[$k1]['business_parent'][$k2]['contract_name'] = "是"; //合同确认（是/否）
				}else{
					$member[$k1]['business_parent'][$k2]['contract_name'] = "否"; //合同确认（是/否）
				}	
			}
		//绑定关系第二次循环 end
		///////计算绩效指标考核（合计总指标、可发放指标、待申请指标、年框KPI）begin
	    foreach($member as $k3 => $v3){
			...
			//绑定关系第一次循环 end
			//绑定关系第二次循环 begin
	    	$member[$k3]['tj_zhibiaos'] = $model
				->field("t.*,b.business_wfk,b.member_id,b.create_time")
				->table("zhiyan_business b, zhiyan_business_time t")
				->where("b.business_del=1 and b.business_id=t.business_id and b.member_id='{$v3['parent_id']}'".$str['str'])
				->select();
			//查询签约合同通过合同
			$signs = M("sign")
				->field("sign_id")
				->where("sign_del=1 and sign_status=2 and member_id='{$v3['parent_id']}'".$str1['str'])
				->select();
			$member[$k3]['qianque_fafangs'] = count($signs)*0.6; //可发放指标（年框）
			//查询签约合同申请中合同
			$signs = M("sign")
				->field("sign_id")
				->where("sign_del=1 and sign_status=1 and member_id='{$v3['parent_id']}'".$str1['str'])
				->select();
			$member[$k3]['qianque_shenqingzhongs'] = count($signs)*0.6; //待申请指标（年框）
			////////////////////////////////////////////////////////查询年框公司
			$member[$k3]['sign_companys'] = $model
				->field("c.customer_company")
				->table("zhiyan_sign s, zhiyan_customer c")
				->where("s.sign_status=2 and s.member_id='{$v3['parent_id']}' and s.customer_id=c.customer_id")
				->select();
			//发放变量设置	
			$zhibiao_fafang_totals = 0;
			$zhibiao_shenqingzhong_totals = 0;
			$zhibiao_fafang1s = 0;
			$zhibiao_fafang2s = 0;
			$zhibiao_fafang3s = 0;
			$zhibiao_fafang4s = 0;
			$zhibiao_shenqingzhong1s = 0;
			$zhibiao_shenqingzhong2s = 0;
			$zhibiao_shenqingzhong3s = 0;
			$zhibiao_shenqingzhong4s = 0;
			//定义总金额变量
			$first_price_totals = 0;
			$end_price_totals = 0;
			$first_price1s = 0;
			$end_price1s = 0;
			$first_price2s = 0;
			$end_price2s = 0;
			foreach($member[$k3]['tj_zhibiaos'] as $k44 => $v44){
				//查询成员所有渠道编号 
				$channels = M("channel")
					->where("member_id='{$v44['member_id']}'")
					->getField("channel_id",true);
				//判断是发放还是申请中（发放）
				if($v44['business_wfk'] == 0){
					///////////////////////////可发放指标（总金额）
					if($v44['time_text'] == 'first'){
						///////////////////////////可发放指标（总金额）
						$first1s = M("channel")
							->where("channel_id='{$v44['channel_id']}'")
							->find();
						$first_price1s = $first_price1s + $first1s['price_first'];
					}else{
						///////////////////////////可发放指标（总金额）
						$end1s = M("channel")
							->where("channel_id='{$v44['channel_id']}'")
							->find();
						$end_price1s = $end_price1s + $end1s['price_end'];	
					}
					//判断是否本人渠道 
					if(in_array($v44['channel_id'],$channels)){
						if($v44['time_text'] == 'first'){
							$zhibiao_fafang1s = $zhibiao_fafang1s + 2*$v44['time_price'];
						}else{
							$zhibiao_fafang2s = $zhibiao_fafang2s + 1*$v44['time_price'];
							///////////////////////////可发放指标（总金额）
						}
					}else{
						if($v44['time_text'] == 'first'){
							$zhibiao_fafang3s = $zhibiao_fafang3s + 2*0.8*$v44['time_price'];
							///////////////////////////可发放指标（总金额）
						}else{
							$zhibiao_fafang4s = $zhibiao_fafang4s + 1*0.8*$v44['time_price'];
							///////////////////////////可发放指标（总金额）
						}
					}
				//判断是发放还是申请中（申请中）	
					...
					if($v44['time_text'] == 'first'){
						...
						$first2s = M("channel")
							->where("channel_id='{$v44['channel_id']}'")
							->find();
						$first_price2s = $first_price2s + $first2s['price_first'];	
					}else{
						///////////////////////////待申请指标（总金额）
						$end2s = M("channel")
							->where("channel_id='{$v44['channel_id']}'")
							->find();
						$end_price2s = $end_price2s + $end2s['price_end'];
					}

					//判断是否本人渠道 
					if(in_array($v44['channel_id'],$channels)){
						if($v44['time_text'] == 'first'){
							$zhibiao_shenqingzhong1s = $zhibiao_shenqingzhong1s + 2*$v44['time_price'];
							///////////////////////////待申请指标（总金额）
						}else{
							$zhibiao_shenqingzhong2s = $zhibiao_shenqingzhong2s + 1*$v44['time_price'];
							///////////////////////////待申请指标（总金额）
						}
					}else{
						if($v44['time_text'] == 'first'){
							$zhibiao_shenqingzhong3s = $zhibiao_shenqingzhong3s + 2*0.8*$v44['time_price'];
							///////////////////////////待申请指标（总金额）
						}else{
							$zhibiao_shenqingzhong4s = $zhibiao_shenqingzhong4s + 1*0.8*$v44['time_price'];
							///////////////////////////待申请指标（总金额）
						}
					}
				}	
			}
			$member[$k3]['first_price_totals'] = $first_price1 + $end_price1 + $first_price1s + $end_price1s; //可发放指标（总金额）
			$member[$k3]['end_price_totals'] = $first_price2 + $end_price2 + $first_price2s + $end_price2s; //待申请指标（总金额）
			$member[$k3]['price_totals'] = $member[$k3]['first_price_totals'] + $member[$k3]['end_price_totals']; //合计总指标（总金额）
			$zhibiao_fafang_totals = $zhibiao_fafang1s + $zhibiao_fafang2s + $zhibiao_fafang3s + $zhibiao_fafang4s;
			$zhibiao_shenqingzhong_totals = $zhibiao_shenqingzhong1s + $zhibiao_shenqingzhong2s + $zhibiao_shenqingzhong3s + $zhibiao_shenqingzhong4s;
			$member[$k3]['zhibiao_fafang_totals'] = $zhibiao_fafang_totals; //可发放指标（指标）
			$member[$k3]['zhibiao_shenqingzhong_totals'] = $zhibiao_shenqingzhong_totals; //待申请指标（指标）
			$member[$k3]['total_zhibiaos'] = $member[$k3]['zhibiao_fafang_totals'] + $member[$k3]['zhibiao_shenqingzhong_totals']; //合计总指标（指标）
			$member[$k3]['total_qianyues'] = $member[$k3]['qianque_fafangs'] + $member[$k3]['qianque_shenqingzhongs']; //合计总指标（年框）
			//绑定关系第二次循环 end

			//绑定关系计算指标 begin
			$member[$k3]['total_zhibiaoss'] = $member[$k3]['total_zhibiao'] + $member[$k3]['total_zhibiaos'];
			$member[$k3]['total_qianyuess'] = $member[$k3]['total_qianyue'] + $member[$k3]['total_qianyues'];
			$member[$k3]['zhibiao_fafang_totalss'] = $member[$k3]['zhibiao_fafang_total'] + $member[$k3]['zhibiao_fafang_totals'];
			$member[$k3]['qianque_fafangss'] = $member[$k3]['qianque_fafang'] + $member[$k3]['qianque_fafangs'];
			$member[$k3]['zhibiao_shenqingzhong_totalss'] = $member[$k3]['zhibiao_shenqingzhong_total'] + $member[$k3]['zhibiao_shenqingzhong_totals'];
			$member[$k3]['qianque_shenqingzhongss'] = $member[$k3]['qianque_shenqingzhong'] + $member[$k3]['qianque_shenqingzhongs'];
			$s3 = M("system")->where("id=3")->find();
			$s1 = M("system")->where("id=1")->find();
			$member[$k3]['zhibiao_zhuli_total'] = ($member[$k3]['total_zhibiao']+$member[$k3]['total_qianyue'])-$s3['left']; //合计总指标（助理超额KPI）
			$member[$k3]['zhibiao_jingli_total'] = ($member[$k3]['total_zhibiaos']+$member[$k3]['total_qianyues'])-$s1['left']; //合计总指标（经理超额KPI）
			$member[$k3]['zhibiao_fafang_zhuli'] = ($member[$k3]['zhibiao_fafang_total']+$member[$k3]['qianque_fafang'])-$s3['left'];
			$member[$k3]['zhibiao_fafang_jingli'] = ($member[$k3]['zhibiao_fafang_totals']+$member[$k3]['qianque_fafangs'])-$s1['left'];
			$member[$k3]['zhibiao_shenqingzhong_zhuli'] = ($member[$k3]['zhibiao_shenqingzhong_total']+$member[$k3]['qianque_shenqingzhong'])-$s3['left'];
			$member[$k3]['zhibiao_shenqingzhong_jingli'] = ($member[$k3]['zhibiao_shenqingzhong_totals']+$member[$k3]['qianque_shenqingzhongs'])-$s1['left'];
			//绑定关系计算指标 end

			//计算 超额KPI 绩效奖金 总金额 begin
			//合计总指标绩效奖金
			$system = M("system")->where("id=2")->find();
			$jixiao_totals = ($member[$k3]['total_zhibiao']+$member[$k3]['total_qianyue'] + $member[$k3]['total_zhibiaos']+$member[$k3]['total_qianyues'])-$system['left']; //合计总指标（超额）
			$member[$k3]['jixiao_totals'] = $jixiao_totals; //合计总指标（超额）
			if($jixiao_totals >= 0 && $jixiao_totals < $system['left']){
				$member[$k3]['jixiao_chaochu_totals'] = 0; //合计总指标（绩效奖金）
				$member[$k3]['jixiao_chaochu_jingli_totals'] = 0;
				$member[$k3]['jixiao_chaochu_zhuli_totals'] = 0;
			}else if($jixiao_totals >= $system['left'] && $jixiao_totals < $system['middle']){
				$member[$k3]['jixiao_chaochu_totals'] = ($jixiao_totals-$system['left'])*$system['left_money']; //合计总指标（绩效奖金）
				$member[$k3]['jixiao_chaochu_jingli_totals'] = $member[$k3]['jixiao_chaochu_totals']*($member[$k3]['zhibiao_jingli_total']/$member[$k3]['jixiao_totals']);
				$member[$k3]['jixiao_chaochu_zhuli_totals'] = $member[$k3]['jixiao_chaochu_totals']*($member[$k3]['zhibiao_zhuli_total']/$member[$k3]['jixiao_totals']);
			}else if($jixiao_totals >= $system['middle'] && $jixiao_totals < $system['right']){
				$member[$k3]['jixiao_chaochu_totals'] = ($jixiao_totals-$system['middle'])*$system['middle_money']+($system['middle']-$system['left'])*$system['left_money']; //合计总指标（绩效奖金）
				$member[$k3]['jixiao_chaochu_jingli_totals'] = $member[$k3]['jixiao_chaochu_totals']*($member[$k3]['zhibiao_jingli_total']/$member[$k3]['jixiao_totals']);
				$member[$k3]['jixiao_chaochu_zhuli_totals'] = $member[$k3]['jixiao_chaochu_totals']*($member[$k3]['zhibiao_zhuli_total']/$member[$k3]['jixiao_totals']);
			}else if($jixiao_totals >= $system['right']){
				$member[$k3]['jixiao_chaochu_totals'] = ($jixiao_totals-$system['right'])*$system['right_money']+($system['right']-$system['middle'])*$system['middle_money']+($system['middle']-$system['left'])*$system['left_money']; //合计总指标（绩效奖金）
				$member[$k3]['jixiao_chaochu_jingli_totals'] = $member[$k3]['jixiao_chaochu_totals']*($member[$k3]['zhibiao_jingli_total']/$member[$k3]['jixiao_totals']);
				$member[$k3]['jixiao_chaochu_zhuli_totals'] = $member[$k3]['jixiao_chaochu_totals']*($member[$k3]['zhibiao_zhuli_total']/$member[$k3]['jixiao_totals']);
			}
			//可发放指标绩效奖金
			$fafang_totals = ($member[$k3]['zhibiao_fafang_total'] + $member[$k3]['qianque_fafang'] + $member[$k3]['zhibiao_fafang_totals'] + $member[$k3]['qianque_fafangs'])-$system['left']; //可发放指标（超额）
			$member[$k3]['jixiao_fafang_totals'] = $fafang_totals; //可发放指标（超额）
			if($fafang_totals >= 0 && $fafang_totals < $system['left']){
				$member[$k3]['fafang_chaochu_totals'] = 0; //可发放指标（绩效奖金）
				$member[$k3]['fafang_chaochu_jingli_totals'] = 0; 
				$member[$k3]['fafang_chaochu_zhuli_totals'] = 0; 
			}else if($fafang_totals >= $system['left'] && $fafang_totals < $system['middle']){
				$member[$k3]['fafang_chaochu_totals'] = ($fafang_totals-$system['left'])*$system['left_money']; //可发放指标（绩效奖金）
				$member[$k3]['fafang_chaochu_jingli_totals'] = $member[$k3]['fafang_chaochu_totals']*($member[$k3]['zhibiao_jingli_total']/$member[$k3]['fafang_totals']); 
				$member[$k3]['fafang_chaochu_zhuli_totals'] = $member[$k3]['fafang_chaochu_totals']*($member[$k3]['zhibiao_zhuli_total']/$member[$k3]['fafang_totals']);
			}else if($fafang_totals >= $system['middle'] && $fafang_totals < $system['right']){
				$member[$k3]['fafang_chaochu_totals'] = ($fafang_totals-$system['middle'])*$system['middle_money']+($system['middle']-$system['left'])*$system['left_money']; //可发放指标（绩效奖金）
				$member[$k3]['fafang_chaochu_jingli_totals'] = $member[$k3]['fafang_chaochu_totals']*($member[$k3]['zhibiao_jingli_total']/$member[$k3]['fafang_totals']); 
				$member[$k3]['fafang_chaochu_zhuli_totals'] = $member[$k3]['fafang_chaochu_totals']*($member[$k3]['zhibiao_zhuli_total']/$member[$k3]['fafang_totals']);
			}else if($fafang_totals >= $system['right']){
				$member[$k3]['fafang_chaochu_totals'] = ($fafang_totals-$system['right'])*$system['right_money']+($system['right']-$system['middle'])*$system['middle_money']+($system['middle']-$system['left'])*$system['left_money']; //可发放指标（绩效奖金）
				$member[$k3]['fafang_chaochu_jingli_totals'] = $member[$k3]['fafang_chaochu_totals']*($member[$k3]['zhibiao_jingli_total']/$member[$k3]['fafang_totals']); 
				$member[$k3]['fafang_chaochu_zhuli_totals'] = $member[$k3]['fafang_chaochu_totals']*($member[$k3]['zhibiao_zhuli_total']/$member[$k3]['fafang_totals']);
			}
			//待申请指标绩效奖金
			$shenqingzhong_totals = ($member[$k3]['zhibiao_shenqingzhong_total'] + $member[$k3]['qianque_shenqingzhong'] + $member[$k3]['zhibiao_shenqingzhong_totals'] + $member[$k3]['qianque_shenqingzhongs'])-$system['left']; //待申请指标（超额）
			$member[$k3]['jixiao_shenqingzhong_totals'] = $shenqingzhong_totals; //待申请指标（超额）
			if($shenqingzhong_totals >= 0 && $shenqingzhong_totals < $system['left']){
				$member[$k3]['shenqingzhong_chaochu_totals'] = 0; //待申请指标（绩效奖金）
				$member[$k3]['shenqingzhong_chaochu_jingli_totals'] = 0;
				$member[$k3]['shenqingzhong_chaochu_zhuli_totals'] = 0;
			}else if($shenqingzhong_totals >= $system['left'] && $shenqingzhong_totals < $system['middle']){
				$member[$k3]['shenqingzhong_chaochu_totals'] = ($shenqingzhong_totals-$system['left'])*$system['left_money']; //待申请指标（绩效奖金）
				$member[$k3]['shenqingzhong_chaochu_jingli_totals'] = $member[$k3]['shenqingzhong_chaochu_totals']*($member[$k3]['zhibiao_jingli_total']/$member[$k3]['shenqingzhong_totals']); 
				$member[$k3]['shenqingzhong_chaochu_zhuli_totals'] = $member[$k3]['shenqingzhong_chaochu_totals']*($member[$k3]['zhibiao_zhuli_total']/$member[$k3]['shenqingzhong_totals']);
			}else if($shenqingzhong_totals >= $system['middle'] && $shenqingzhong_totals < $system['right']){
				$member[$k3]['shenqingzhong_chaochu_totals'] = ($shenqingzhong_totals-$system['middle'])*$system['middle_money']+($system['middle']-$system['left'])*$system['left_money']; //待申请指标（绩效奖金）
				$member[$k3]['shenqingzhong_chaochu_jingli_totals'] = $member[$k3]['shenqingzhong_chaochu_totals']*($member[$k3]['zhibiao_jingli_total']/$member[$k3]['shenqingzhong_totals']); 
				$member[$k3]['shenqingzhong_chaochu_zhuli_totals'] = $member[$k3]['shenqingzhong_chaochu_totals']*($member[$k3]['zhibiao_zhuli_total']/$member[$k3]['shenqingzhong_totals']);
			}else if($shenqingzhong_totals >= $system['right']){
				$member[$k3]['shenqingzhong_chaochu_totals'] = ($shenqingzhong_totals-$system['right'])*$system['right_money']+($system['right']-$system['middle'])*$system['middle_money']+($system['middle']-$system['left'])*$system['left_money']; //待申请指标（绩效奖金）
				$member[$k3]['shenqingzhong_chaochu_jingli_totals'] = $member[$k3]['shenqingzhong_chaochu_totals']*($member[$k3]['zhibiao_jingli_total']/$member[$k3]['shenqingzhong_totals']); 
				$member[$k3]['shenqingzhong_chaochu_zhuli_totals'] = $member[$k3]['shenqingzhong_chaochu_totals']*($member[$k3]['zhibiao_zhuli_total']/$member[$k3]['shenqingzhong_totals']);
			}
			//p($system);
	...
	public function get_business_member(){
