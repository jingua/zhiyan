	...
	public function excel(){
        ...
        foreach($arr as $v1){
			...
			$objPHPExcel->getActiveSheet()
					...
					->setBorderStyle(\PHPExcel_Style_Border::BORDER_THIN);
			$objSheet
				->setCellValue("A".$z,$v1['member_text']);
			$y1 = $z+10;
			$y2 = $z+11;
			$y3 = $z+12;
			$y4 = $z+13;
			...
			$objSheet
				...
				foreach($v1['sign_company'] as $v3){
					...
					$objSheet
						->setCellValue("A".$y6,$v3['customer_company']);
						$y6++;
				}
			$i++;
        }
        ...
        if($v){
        	$objPHPExcel->createSheet();
        	$objPHPExcel->setActiveSheetIndex($v-2);
			$objSheet=$objPHPExcel->getActiveSheet();
	        $objSheet->setTitle("渠道统计");
	        $jj = 2;
	        $ss = $jj-1;
	        foreach($channel as $v5){
	        	...
	        	$jj = $jj+1;
	        	$objSheet
					->setCellValue("A".$aa,$v5['channel_name']."(".$from."—".$to.")");
				$objSheet->mergeCells("A".$aa.":"."G".$aa);
				$objPHPExcel->getActiveSheet()
					->getStyle("A".$aa)
					->getAlignment()
					->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_CENTER); 
        ...
        foreach($parent as $v7){
			$objPHPExcel->createSheet();
			$objPHPExcel->setActiveSheetIndex($ii-2);
			$objSheet=$objPHPExcel->getActiveSheet();
			$objSheet->setTitle($v7['name']);
			//begin
			$objSheet->mergeCells("A1".":"."U1");
			$objPHPExcel->getActiveSheet()
				->getStyle('A1')
			...
			foreach($v7['business'] as $v8){
				$objSheet
					->setCellValue("A".$j,$v8['update_time'])
			...
			foreach($v7['business_parent'] as $v9){
				$objSheet
					->setCellValue("A".$j,$v9['update_time'])
			...
			$objSheet
				->setCellValue("A".$y2,"合计总指标")
				->setCellValue("B".$y2,$v7['total_zhibiao'])
				->setCellValue("C".$y2,$v7['total_qianyue'])
				->setCellValue("D".$y2,$v7['jixiao_total'])
				->setCellValue("E".$y2,$v7['jixiao_chaochu_total'])
				->setCellValue("F".$y2,$v7['price_total']);
			$objSheet
				->setCellValue("A".$y3,"可发放指标")
				->setCellValue("B".$y3,$v7['zhibiao_fafang_total'])
				->setCellValue("C".$y3,$v7['qianque_fafang'])
				->setCellValue("D".$y3,$v7['jixiao_fafang_total'])
				->setCellValue("E".$y3,$v7['fafang_chaochu_total'])
				->setCellValue("F".$y3,$v7['first_price_total']);
			$objSheet
				->setCellValue("A".$y4,"待申请指标")
				->setCellValue("B".$y4,$v7['zhibiao_shenqingzhong_total'])
				->setCellValue("C".$y4,$v7['qianque_shenqingzhong'])
				->setCellValue("D".$y4,$v7['jixiao_shenqingzhong_total'])
				->setCellValue("E".$y4,$v7['shenqingzhong_chaochu_total'])
				->setCellValue("F".$y4,$v7['end_price_total']);
			//年框公司
			$objSheet->mergeCells("A".$y5.":"."F".$y5);
			$objPHPExcel->getActiveSheet()
				->getStyle("A".$y5)
